<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:63:"C:\PHP\php11\WWW\Patient/Admin/score\view\score\midlegedit.html";i:1523944673;s:73:"C:\PHP\php11\WWW\Patient/Admin/score\view\..\..\com\view\public\meta.html";i:1521619099;s:75:"C:\PHP\php11\WWW\Patient/Admin/score\view\..\..\com\view\public\footer.html";i:1523166087;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
...
<article class="cl pd-20">
	...
	<hr>
	<input type="hidden" value="<?php echo $list['midleg_id']; ?>" name="midleg_id">
			...
			<label for="name" class="lh-30 ml-40"><strong>穿鞋需求：</strong></label><br>
				<label class="radio-inline ml-50 lh-26">
					<input type="radio" name="shoes" value="5" <?php if(($list['shoes']==5)): ?>checked<?php endif; ?>>1、时尚、休闲，无需内垫
				</label><br>
				<label class="radio-inline ml-50 lh-26">
					<input type="radio" name="shoes" value="3" <?php if(($list['shoes']==3)): ?>checked<?php endif; ?>> 2、舒适鞋，需内垫 
				</label><br>
				<label class="radio-inline ml-50 lh-26">
			...
			<label for="name" class="lh-30 ml-40"><strong>最大行走距离（街区，1街区约200米）： </strong></label><br>
				<label class="radio-inline ml-50 lh-26">
					<input type="radio" name="distance" value="10" <?php if(($list['distance']==10)): ?>checked<?php endif; ?>>1、大于6街区（约1200米） 
				</label><br>
				<label class="radio-inline ml-50 lh-26">
					<input type="radio" name="distance" value="7" <?php if(($list['distance']==7)): ?>checked<?php endif; ?>>2、 4-6街区 
				</label><br>
				<label class="radio-inline ml-50 lh-26">
					<input type="radio" name="distance" value="4" <?php if(($list['distance']==4)): ?>checked<?php endif; ?>>3、1-3街区
				</label><br>
				<label class="radio-inline ml-50 lh-26">
			...
			<label for="name" class="lh-30 ml-40"><strong>行走路面：</strong></label><br>
				<label class="radio-inline ml-50 lh-26">
					<input type="radio" name="pavement" value="10" <?php if(($list['pavement']==10)): ?>checked<?php endif; ?>>1、任何路面无困难 
				</label><br>
				<label class="radio-inline ml-50 lh-26">
					<input type="radio" name="pavement" value="5" <?php if(($list['pavement']==5)): ?>checked<?php endif; ?>>2、不平路面、台阶、下坡、梯子有一些困难 
				</label><br>
				<label class="radio-inline ml-50 lh-26">
			...
			<label for="name" class="lh-30 ml-40"><strong>步态异常： </strong></label><br>
				<label class="radio-inline ml-50 lh-26">
					<input type="radio" name="gait" value="10" <?php if(($list['gait']==10)): ?>checked<?php endif; ?>>1、大于6街区（约1200米） 
				</label><br>
				<label class="radio-inline ml-50 lh-26">
					<input type="radio" name="gait" value="5" <?php if(($list['gait']==5)): ?>checked<?php endif; ?>>2、有明显异常
				</label><br>
				<label class="radio-inline ml-50 lh-26">
...
function submit_form(){
	var len=$("input:radio:checked").length;
		if(len==7){
			...
			var Total=parseInt(Line_of_force)+parseInt(pain)+parseInt(auxiliary)+parseInt(distance)+parseInt(pavement)+parseInt(gait)+parseInt(shoes);
			layer.confirm('合计得分：'+Total+'<br>点击【确定】将保存该次调研。', function(){
				 $.ajax({
            		type:"POST",
            		url:"<?php echo url('midlegedit'); ?>",
